...
is_twice_as_long("yes", "forsure")
is_twice_as_long("forsure", "yes")
...
describe_difference <- function(first, second) {
  if (nchar(first) > nchar(second)) {
    n <- nchar(first) - nchar(second)
    return(paste("Your first string is longer by", n, "characters"))
  } else if (nchar(second) > nchar(first)) {
    n <- nchar(second) - nchar(first)
